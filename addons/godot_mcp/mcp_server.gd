...
func _process_http_request(client: StreamPeerTCP) -> void:
	...
	if method == "POST" and path == "/mcp":
		...
		response = _create_method_not_allowed_response()
	elif method == "GET" and _is_oauth_discovery_path(path):
		response = _create_oauth_discovery_response()
	elif method == "GET" and _is_oauth_protected_resource_path(path):
		response = _create_oauth_protected_resource_response(path)
...
func _create_mcp_endpoint_response() -> Dictionary:
	return {
		"status": 200,
		"name": SERVER_NAME,
		"version": SERVER_VERSION,
...
func _create_method_not_allowed_response() -> Dictionary:
	return {
		"status": 405,
...
func _create_oauth_discovery_response() -> Dictionary:
	var issuer = "http://%s:%d" % [_host, _port]
	return {
		"status": 200,
		"issuer": issuer,
		...
		"service_documentation": "MCP server exposed by Godot editor plugin",
		"auth": {
			"type": "none"
		}
	}
...
func _create_oauth_protected_resource_response(path: String) -> Dictionary:
	var issuer = "http://%s:%d" % [_host, _port]
...
func _is_oauth_discovery_path(path: String) -> bool:
	return path in [
...
func _is_oauth_protected_resource_path(path: String) -> bool:
	return path in [
...
func _send_http_response(client: StreamPeerTCP, data: Dictionary) -> void:
	...
	# HTTP 204 must not include a response body.
	if status_code != 204:
		body = JSON.stringify(sanitized)
		body_bytes = body.to_utf8_buffer()

	var status_texts = {200: "OK", 204: "No Content", 404: "Not Found", 405: "Method Not Allowed", 500: "Internal Server Error"}
	...
	var err2 = OK
	if status_code != 204 and body_bytes.size() > 0:
